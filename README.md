...
*   **System Peripherals:** Onboard physical SPDT slide switch (`S1`) dedicated to isolating the battery power rail, built-in hardware RC reset circuit, hardware boot-option selector switch (`IO9`), and compliant $5.1\text{k}\Omega$ configuration channel (CC) pull-down resistors for USB-C power delivery negotiation.
